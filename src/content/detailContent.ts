...
export const detailPages: DetailPage[] = [
  {
    ...
    sections: [
      ...
      { id: 'integration', title: '既有 ERP、POS、CRM，如何接上 AI？', body: '從可用資料庫、API 與企業知識開始介接，保留原系統已建立的資料與業務規則。資料查詢、單據交換、資料拋轉，依原廠介面、授權及交易要求逐項確認；工程師補齊資料對應與必要的擴充工具。', points: ['快速建置：以現有資料與通用生成能力切入高價值場景。', 'LINE 生態整合：依情境配置對話、LIFF、表單、報表及身分綁定入口。', '業態專屬技能：讓 AI 使用企業的術語、資料字典與作業知識。', '企業流程框架：明確定義工作步驟、輸出規格與檢核規則。'] },
