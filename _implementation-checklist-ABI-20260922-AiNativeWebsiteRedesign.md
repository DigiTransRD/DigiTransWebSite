...
- [x] 全站底部左側提供「導入方案」、右側提供精簡「預約評估」懸浮按鈕，移除首屏原導入方案按鈕；預留頁腳及行動版安全區空間，驗證兩個入口、手機並排及各頁最後一行未受遮蔽；依 PM 截圖將「預約評估」連結指定到 /contact/#contact-form，直接定位表單標題，保留同頁已填內容。
...
- 預約評估定位追加驗證：僅調整 App.vue 懸浮入口與 ContactSection.vue 表單 id，沿用既有 Router 捲動與頁首避讓，不修改介紹區、表單送出或 API。TypeScript／Vite／SSR／21 頁建置通過。
- 正式成品於 320／390／600／768／1440px 驗證首頁到表單、同網址重複點擊，另檢查技術詳頁跨頁導入、直接帶 hash 重新載入、contact 頁已填欄位保留；表單頂端在固定頁首下方約 24px，無橫向溢出，390px 截圖已視覺檢視。互動過程無 pageerror，未實際送出表單、未發布。
